...
class CustomEnv:
    def __init__(self, rows=10, columns=10, start_position=(0, 0), goal_position=(7, 2), reward=1.0,
        ...
        # Fuzzy variables
        self.dist_x = ctrl.Antecedent(np.arange(0, 11, 1), 'dist_x')
        self.dist_y = ctrl.Antecedent(np.arange(0, 11, 1), 'dist_y')
        self.reward_fuzzy = ctrl.Consequent(np.arange(-10, 11, 1), 'reward')

        # Distance in x direction
        self.dist_x['very_close'] = fuzz.trimf(self.dist_x.universe, [0, 0, 3])
        self.dist_x['close'] = fuzz.trimf(self.dist_x.universe, [1, 3, 5])
        self.dist_x['medium'] = fuzz.trimf(self.dist_x.universe, [3, 5, 7])
        self.dist_x['far'] = fuzz.trimf(self.dist_x.universe, [5, 7, 9])

        # Distance in y direction
        self.dist_y['very_close'] = fuzz.trimf(self.dist_y.universe, [0, 0, 3])
        self.dist_y['close'] = fuzz.trimf(self.dist_y.universe, [1, 3, 5])
        self.dist_y['medium'] = fuzz.trimf(self.dist_y.universe, [3, 5, 7])
        self.dist_y['far'] = fuzz.trimf(self.dist_y.universe, [5, 9, 9])

        self.reward_fuzzy['negativeS'] = fuzz.trimf(self.reward_fuzzy.universe, [-10, -10, -5])
        self.reward_fuzzy['negative'] = fuzz.trimf(self.reward_fuzzy.universe, [-5, -5, -2])
        self.reward_fuzzy['zero'] = fuzz.trimf(self.reward_fuzzy.universe, [-5, 0, 5])
        self.reward_fuzzy['positive'] = fuzz.trimf(self.reward_fuzzy.universe, [0, 10, 10])

       # Fuzzy rules
        self.rule1 = ctrl.Rule(self.dist_x['very_close'] & self.dist_y['very_close'], self.reward_fuzzy['positive'])
        self.rule2 = ctrl.Rule(self.dist_x['very_close'] & self.dist_y['close'], self.reward_fuzzy['positive'])
        self.rule3 = ctrl.Rule(self.dist_x['very_close'] & self.dist_y['medium'], self.reward_fuzzy['negative'])
        self.rule4 = ctrl.Rule(self.dist_x['very_close'] & self.dist_y['far'], self.reward_fuzzy['negativeS'])

        self.rule5 = ctrl.Rule(self.dist_x['close'] & self.dist_y['very_close'], self.reward_fuzzy['positive'])
        self.rule6 = ctrl.Rule(self.dist_x['close'] & self.dist_y['close'], self.reward_fuzzy['positive'])
        self.rule7 = ctrl.Rule(self.dist_x['close'] & self.dist_y['medium'], self.reward_fuzzy['negative'])
        self.rule8 = ctrl.Rule(self.dist_x['close'] & self.dist_y['far'], self.reward_fuzzy['negativeS'])

        self.rule9 = ctrl.Rule(self.dist_x['medium'] & self.dist_y['very_close'], self.reward_fuzzy['negative'])
        self.rule10 = ctrl.Rule(self.dist_x['medium'] & self.dist_y['close'], self.reward_fuzzy['negative'])
        self.rule11 = ctrl.Rule(self.dist_x['medium'] & self.dist_y['medium'], self.reward_fuzzy['negativeS'])
        self.rule12 = ctrl.Rule(self.dist_x['medium'] & self.dist_y['far'], self.reward_fuzzy['negativeS'])

        self.rule13 = ctrl.Rule(self.dist_x['far'] & self.dist_y['very_close'], self.reward_fuzzy['negative'])
        self.rule14 = ctrl.Rule(self.dist_x['far'] & self.dist_y['close'], self.reward_fuzzy['negativeS'])
        self.rule15 = ctrl.Rule(self.dist_x['far'] & self.dist_y['medium'], self.reward_fuzzy['negativeS'])
        self.rule16 = ctrl.Rule(self.dist_x['far'] & self.dist_y['far'], self.reward_fuzzy['negativeS'])


        # Add all rules to the control system
        self.reward_ctrl = ctrl.ControlSystem([
        ...
        self.reward_sim = ctrl.ControlSystemSimulation(self.reward_ctrl)
    ...
    def step(self, action):
        ...
        # Calculate fuzzy reward
        self.reward_sim.input['dist_x'] = abs(self.current_position[1] - self.goal_position[1])
        self.reward_sim.input['dist_y'] = abs(self.current_position[0] - self.goal_position[0])
        self.reward_sim.compute()
        fuzzy_reward = self.reward_sim.output['reward']

        return self.current_position, fuzzy_reward, done
...
def update_plot3D_thread():
    ...
    while 1:
        ...
        # New plot
        x, y = np.meshgrid(range(env.rows), range(env.columns))
        z = np.max(env.q_table, axis=2)
...
def tk_thread():
    ...
    delay_slider = Scale(root, from_=0.0000001, to=1, orient="horizontal", resolution=0.0001, command=update_delay, length=300, sliderlength=50)
    delay_slider.set(0.0000001)
...
for run in range(5):
    ...
    filename = f'runs3/{timestamp}_{random_number}.csv'
